 ...
 * Pass `scope` for tables whose slug unique index is composite — e.g.
 * ticket_tiers uses (event_id, slug), so `scope: { column: "event_id", value: eventId }`
 * narrows the uniqueness check to that event.
 */
